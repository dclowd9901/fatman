  ...
  EntityParticleEmitter = ig.Entity.extend({
    ...
    emit : function (){
      ...
      if( this.direction === 'down' ){
        ig.game.spawnEntity('EntityDust', spawn.x, spawn.y, { velocity : 0, lifetime : this.lifetime });
      }

      if( this.direction === 'right' ){
        velocity = Math.random().map( 0, 1, 1, 50 );
        ig.game.spawnEntity('EntityDust', spawn.x, spawn.y, { velocity : velocity, lifetime : this.lifetime });
      }
